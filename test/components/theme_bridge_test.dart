...
void main() {
  ...
  group('主题桥接', () {
    testWidgets('material_ui 侧通过桥接读到的是品牌主色，不是 fallback', (tester) async {
      ...
      expect(got.primary.toARGB32(), Themes.brand.toARGB32(),
          reason: 'material_ui 侧拿到的不是品牌色 —— 桥接没生效，'
              '它静默退回了 fallback（这正是最难发现的那种失败）');
      expect(got.primary.toARGB32(), isNot(fallbackPrimary.toARGB32()),
    ...
    testWidgets('没有桥接时会静默退回 fallback（说明这层不是可选的）', (tester) async {
      ...
      expect(got.primary.toARGB32(), fallbackPrimary.toARGB32(),
          reason: '若这里不再是 fallback，说明 Flutter 改了跨库行为，'
  ...
  group('主题一致性', () {
    test('flutter 版与 material_ui 版主色一致', () {
      expect(Themes.light.colorScheme.primary.toARGB32(),
          Themes.muiLight.colorScheme.primary.toARGB32());
      expect(Themes.dark.colorScheme.primary.toARGB32(),
          Themes.muiDark.colorScheme.primary.toARGB32());
    });

    test('两套主题都是品牌色（没有被 flex9 的默认方案覆盖）', () {
      expect(
          Themes.light.colorScheme.primary.toARGB32(), Themes.brand.toARGB32());
      expect(
          Themes.dark.colorScheme.primary.toARGB32(), Themes.brand.toARGB32());
    });
  ...
  group('本地化桥接（smart_dialog 5.3 的 dialog 必需）', () {
    testWidgets('mui 的 MaterialLocalizations 通过桥接可取到', (tester) async {
      ...
      expect(caught, isNull,
          reason: '取 mui MaterialLocalizations 抛异常了 —— smart_dialog 的 '
    ...
    testWidgets('没有桥接时取不到 mui 的 MaterialLocalizations（说明这层必需）', (tester) async {
      ...
      expect(caught, isNotNull,
          reason: '若这里不再抛异常，说明 mui 的本地化变得可继承，'
